    ...
    for mode in modes:
        ...
        with open ( os.path.join(anno_root, f'person_keypoints_{mode}.json') , 'r') as json_file:
            ...
            for elem in f["annotations"]:
                ...
                elem["keypoints"][0::3] = np.ndarray.tolist(width-x_key)
                elem["keypoints"][1::3] = np.ndarray.tolist(height- y_key)
                '''
                #rotate segmentation
                x_seg = np.array(elem["segmentation"][0::2])
                y_seg = np.array(elem["segmentation"][1::2])
                elem["segmentation"][0::2] = np.ndarray.tolist(width - x_seg)
                elem["segmentation"][1::2] = np.ndarray.tolist(height - y_seg)
                '''
                #rotate bbox
                elem["bbox"][0] = width - elem["bbox"][0] - elem["bbox"][2]
                elem["bbox"][1] = height - elem["bbox"][1] - elem["bbox"][3]
